...
export async function POST(req: Request) {
  try {
    ...
    if (!sellerEmail) {
      return NextResponse.json({ error: 'No email field in message' }, { status: 400 });
    }
    ...
    const mailOptions = {
      //
      from:  `"my-market-25kc Support" <${process.env.EMAIL_USER}>`,
      to: sellerEmail,
        ...
        <div style="font-family: Arial, sans-serif; padding: 24px; background-color: #f8f9fb; border-radius: 10px;">
          <div style="max-width: 500px; margin: auto; background: white; padding: 24px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${setPasswordUrl}" style="display: inline-block; background-color: #5e3e89; color: white; text-decoration: none; padding: 12px 28px; border-radius: 6px; font-weight: 600; font-size: 15px;">
                Set Password
              </a>
            </div>
